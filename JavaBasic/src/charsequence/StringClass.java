 ...
 * String 有两种声明方式:
 * 1. "..."(字面量)  存放在常量池
 * 2. new String();(实例) 存放在堆中
 * 这两种方式均会生成至少1个新的 String 实例
 * <p>
 * 变量类型 变量 赋值(=) 数据;
 * 对于基本类型, 变量存储的数据是数值本身
 * 对于引用类型, 变量存储的数据是地址
 * 运算符 == 用于比较 运算符两侧的值 是否相等,
 * 对于基本类型比较数值本身是否相等, 对于引用类型比较地址是否相等
 * String类 重写了从 Object继承的 equals 方法
 * <p>
 * 字面量存放规则:
 * 1. 当字面量首次出现时, 会在 String常量池 中查找是否存在相应的 String实例
 * 2. 若 String常量池 中不存在相应的 String实例 , 创建新的实例并添加到 String常量池 , 返回该实例地址
 * 3. 若 String常量池 中存在相应的 String实例 , 直接返回 String常量池 中该实例的地址
 * <p>
 * 1. 有变量参与的字符串连接操作都会返回新的 String 实例, 参见 func2();
 * 2. 当连接字面量时, 会直接拼接该字面量, 新的字面量参考存放规则
 * 3. 当变量被 final修饰时, 此时变量与字面量相同, 连接变量与连接字面量相同, 参见 func3();
 * <p>
