 ...
 *
 * // TODO 修改剩余的 static 说明 静态 -> 类 static{} -> StaticInitializers 类初始化器
 * static 是 Java关键字, 表示 类相关的(不要翻译为: 静态) ,可以修饰 内部类 ,成员属性 和 成员方法, 定义静态块
 * 被 static修饰 的 实例属性 和 实例方法 称为 类属性 和 类方法, 属于 类成员, 不能用 this.
 * e.g.
 * .    // 类属性
 * .    public static int a;
 * .    // 类方法
 ...
 * .    // 类初始化器 (StaticInitializers) (俗称: 静态代码块)
 * .    static{}
 * 1. 类成员 在类首次出现时在方法区中分配内存, 进行 类初始化, 先初始化 类属性 和 类方法, 之后从上至下调用 类初始化器
 * 2. 类成员 只分配一次内存
 * 3. 类成员 由 类名 调用 (反对用 实例名 调用)
 * 4. 类属性 可以通过 类初始化器 进行初始化, (实例属性 可以由 构造函数 初始化), 也可以不初始化,会自动初始化赋默认值
 * <p>
 * 即: this 和 super 都可以调用 实例成员
 * 遵循: (成员)类属性/方法 之间可以互相调用, 成员属性/方法 可以调用 类属性/方法, 类属性/方法 不能调用 成员属性/方法
